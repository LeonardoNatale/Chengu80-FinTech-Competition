...
def readData():
    data_full = pd.read_csv('Average credit card spending across factors.csv', header=0, index_col=0)
    return data_full

def forecast(df_x_inc):
    df_x_inc = df_x_inc[df_x_inc['age'] >= 18]
    df_x_inc = df_x_inc[df_x_inc['age'] < 68]
    df_x_inc = df_x_inc.dropna(axis=0)
    df_x_inc['avg_credit_card_spending_annual'] = df_x_inc['avg_credit_card_spending_semi_annual']*2
    df_x_inc['income_past12months'] = np.where(df_x_inc['income_past12months'] <= 0, 1, df_x_inc['income_past12months'])
    '''
    df_x_inc['avg_credit_card_spending_annual'] = np.where(df_x_inc['avg_credit_card_spending_annual'] <= 0, 1,
                                                           df_x_inc['avg_credit_card_spending_annual'])
    '''
    ...
    df_x_inc = df_x_inc.drop(columns=['avg_credit_card_spending_semi_annual', 'house_tenure', 'education_level', 'occupation', 'sex', 'marital_status'])
    ...
    est = sm.OLS(df_x_inc['avg_credit_card_spending_annual'], df_x_inc.drop(columns=['avg_credit_card_spending_annual'])).fit()
    return est
    '''
    y = df_x_inc[['avg_credit_card_spending_annual']]
    x = df_x_inc.drop(columns=['avg_credit_card_spending_annual'])
    #x = df_x_inc[['age']]
...
def estVarianceT10(data_full, age, edu):
    age += 10
    df_filter = data_full[['age', 'education_level', 'avg_credit_card_spending_semi_annual']]
    df_filter = df_filter[df_filter['age'] <= age + 2]
    df_filter = df_filter[df_filter['age'] >= age - 2]
    df_filter = df_filter[df_filter['education_level'] == edu]
    df_filter['avg_cc_annual'] = df_filter['avg_credit_card_spending_semi_annual']*2
    variance = df_filter['avg_cc_annual'].var()
    mean = df_filter['avg_cc_annual'].mean()
    sigma = math.sqrt(variance)/mean
    return sigma


def forecastIncome(input_array, data_full):
    age = input_array[1]
    edu = input_array[6]
    df_filter = data_full[['age', 'education_level', 'income_past12months']]
    df_filter = df_filter[df_filter['age'] <= age + 2]
    df_filter = df_filter[df_filter['age'] >= age - 2]
    df_filter = df_filter[df_filter['education_level'] == edu]
    mean = df_filter['income_past12months'].mean()
    return mean
